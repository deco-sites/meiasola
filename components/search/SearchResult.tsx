...
function Result({ page, images = [], sizes, isWishlistPage = false, searchQueryParam = "", }: Omit<SectionProps<typeof loader>, "page" | "notFoundPage"> & {
    page: ProductListingPageAndSearch;
}) {
    const { products, filters, breadcrumb, sortOptions, seo, pageInfo, search } = page;
    ...
    return (<>
      {page.isNotFoundPage && (<div class="container relative z-10">
          <Breadcrumb itemListElement={breadcrumb?.itemListElement}/>
        </div>)}

      {!isWishlistPage && !isSearchPage && images && images.length > 0 && (<Image images={images} breadcrumb={breadcrumb} pathname={search.url.pathname}/>)}

      {!isWishlistPage && !isSearchPage && sizes && sizes.sizes.length > 0 && (<Sizes title={sizes.title} routes={sizes.routesToShow} sizes={sizes.sizes} url={search.url}/>)}

      <Heading seo={seo} productsCount={pageInfo.records ?? 0} sortOptions={sortOptions} searchTerm={search?.term ?? undefined} isNotFoundPage={page.isNotFoundPage}/>
      ...
      <div class="container py-6 laptop:py-10 grid grid-cols-4 gap-4 laptop:grid-cols-12 laptop:gap-5">
        <aside class="flex flex-col gap-6 col-span-3">
          <span class={`text-black uppercase text-small font-bold ${search?.term && "laptop:hidden"}`}>
        ...
        <div class="col-span-4 laptop:col-span-9 flex flex-col items-center gap-6 laptop:gap-10">
          ...
          {willPaginate && (<ul class="flex gap-8">
              {pageInfo.currentPage - 2 > 0 && (<PageLink page={pageInfo.currentPage - 2} href={paginationUrl.href.replace("pagination-number", (pageInfo.currentPage - 2).toString())}/>)}

              {pageInfo.previousPage && (<PageLink page={pageInfo.currentPage - 1} href={paginationUrl.href.replace("pagination-number", (pageInfo.currentPage - 1).toString())}/>)}

              <PageLink page={pageInfo.currentPage} isActive href={"#"}/>

              {pageInfo.currentPage * pageInfo.recordPerPage! <
                pageInfo.records! && (<PageLink page={pageInfo.currentPage + 1} href={paginationUrl.href.replace("pagination-number", (pageInfo.currentPage + 1).toString())}/>)}

              {(pageInfo.currentPage + 1) * pageInfo.recordPerPage! <
                pageInfo.records! && (<PageLink page={pageInfo.currentPage + 2} href={paginationUrl.href.replace("pagination-number", (pageInfo.currentPage + 2).toString())}/>)}
            </ul>)}
        </div>
      </div>
      ...
      {isSearchPage && (<script dangerouslySetInnerHTML={{
                __html: `(${updateRecentSearches})("${searchQueryParam}");`,
            }}/>)}
    </>);
}
function Heading({ seo, sortOptions, productsCount, searchTerm, title, isNotFoundPage, }: {
    searchTerm?: string;
    ...
    return (<div class={`${!isNotFoundPage
      ...
      <div class="container flex flex-col gap-6 laptop:flex-row laptop:justify-between laptop:items-end">
        {isNotFoundPage && (<div class="flex flex-col gap-8">
            <h1 class="text-subtile font-normal leading-none">
              <span class="font-bold text-h2">{searchTerm}</span>
            </h1>
          </div>)}

        {searchTerm && !isNotFoundPage ? (<div class="flex flex-col gap-8">
            <h1 class="text-subtile font-normal leading-none">
          ...
          </div>) : (!isNotFoundPage && (<div class="flex flex-col gap-6 laptop:flex-row laptop:w-3/4 laptop:gap-5 laptop:items-center">
...
function Sizes({ title, routes, sizes, url, }: {
    title: string;
    routes: Route[];
    sizes: Size[];
    url: URL;
    ...
    if (!routes.findLast((route) => url.pathname.includes(route.label)))
        return null;
    return (<div class="bg-black text-white text-large py-6 laptop:py-5">
      <div class="container flex flex-col items-center gap-8 laptop:flex-row laptop:justify-between">
        ...
        <ul class="grid grid-cols-4 gap-8 tablet:flex tablet:flex-wrap tablet:justify-center laptop:gap-6 desktop:gap-8">
          {sizes?.map((size, index) => {
            ...
            const isActive = sizeUrl.search.includes(`filter.tamanho=${size.label}`);
            if (isActive)
                sizeUrl.searchParams.delete("filter.tamanho");
            else {
                if (sizeUrl.searchParams.get("filter.tamanho"))
                    sizeUrl.searchParams.set("filter.tamanho", size.label);
                else
                    sizeUrl.searchParams.append("filter.tamanho", size.label);
            }
            return (<li key={"size-" + index}>
                <a href={sizeUrl.href} aria-label={`Numeração ${size.label}`} class={`border border-white h-10 w-10 flex items-center justify-center hover:bg-white hover:text-black ${isActive ? "bg-white text-black" : ""} transition-all duration-300 ease-out`}>
                  {size.label}
                </a>
              </li>);
        })}
...
function Image({ images = [], breadcrumb, pathname, }: {
    images: Image[];
    ...
    if (image) {
        return (<div class="w-full h-[260px] relative py-10">
        <div class="container relative z-10">
          <Breadcrumb itemListElement={breadcrumb?.itemListElement}/>
        </div>
        <Picture>
          <Source media="(max-width: 767px)" src={image.srcMobile} width={390} height={260}/>
          <Source media="(min-width: 768px)" src={image.src} width={1440} height={260}/>
          <img class="h-full w-full object-cover absolute top-0 left-0 z-0" sizes="100vw" src={image.src} alt={image.alt ?? "Imagem da Categoria"} decoding="async" loading="lazy"/>
        </Picture>
      </div>);
    }
    return null;
...
export const loader = async (props: Props, req: Request, ctx: AppContext) => {
    ...
    const notFoundPage = (await notFoundProductListingPage({
        ...
        fuzzy: "automatic",
    }, req, ctx)) as ProductListingPageAndSearch;
    if (notFoundPage) {
...
function SearchResult({ page, notFoundPage, ...props }: SectionProps<typeof loader>) {
...
function NotFound() {
    return (<div class="w-full py-10 laptop:border-b laptop:border-grey-1">
...
function PageLink({ page, isActive = false, href, }: {
    href: string;
    page: number;
    isActive?: boolean;
}) {
    return (<a href={href} aria-label={`Página ${page}`} class={`border border-black h-10 w-10 flex items-center justify-center hover:bg-black hover:text-white ${isActive ? "bg-black text-white" : ""} transition-all duration-300 ease-out`}>
      {page}
    </a>);
}
